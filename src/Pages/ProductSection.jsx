...
const ProductSection = () => {
  ...
  const handleSubmit = async (total, selectedItems, selectedSize, selectedDough, history) => {
    if (!validateForm()) {
      setShowToast(true);
    ...
    try {
      // Axios ile API çağrısı simülasyonu
      const orderData = {
      ...
      console.log('Sipariş başarıyla gönderildi:', result);
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 flex items-center gap-4">
            ...
            <span className="text-gray-700 font-medium">Siparişiniz gönderiliyor...</span>
          </div>
        </div>
      )}
    </div>
